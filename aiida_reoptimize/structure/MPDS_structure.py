...
def get_geometry_MPDS(mpds_query: dict):
    """Retrieve a crystal structure from the MPDS database.

    Queries MPDS for atomic structures matching the given criteria, selects the
    structure with the fewest atoms and median cell vectors.

    Args:
        mpds_query: Dictionary with query parameters (e.g. ``{'formulae': 'WS2', 'sgs': 194}``).

    Returns:
        ASE Atoms object representing the best-matching structure.

    Raises:
        Exception: If the MPDS API key is not set or no structures are found.
    """
    # check for API key
