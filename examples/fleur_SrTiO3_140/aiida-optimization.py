from src.optimizers.PSO import PSOWorkChain

from aiida import load_profile

from aiida.engine import WorkChain, ToContext, run
from aiida.orm import Float, List, Dict, Int, Str, StructureData, load_node
...
class ExamplePSO(PSOWorkChain):
    def evaluate(self):
        ...
        Steps:
        1. Convert particle positions to AiiDA List type
        2. Submit ObjectiveFunctionWorkChain for each position
        3. Track execution status through futures dictionary
        """
        futures = {}
        ...
        for i, pos in enumerate(self.ctx.positions):
            ...
            futures[f'eval_{i}'] = future
        return ToContext(**futures)  # Wait for all processes to complete
...
inputs = {
    'key_value': Str('energy'),  # Передаем класс целевой функции
    'num_particles': Int(1),
...
# Запускаем PSOWorkChain
result = run(ExamplePSO, **inputs)
print("Optimization Results:")
print(f"Best position: {result['best_position']}")
print(f"Best energy: {result['best_value']}")